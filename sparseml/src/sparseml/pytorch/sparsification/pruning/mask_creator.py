...
class HiNMPruningMaskCreator(PruningMaskCreator):
    """
    Class for creating N:M sparsity masks.
    Masks will be created using the N:M ratio, where for every block of M weights,
    N will be pruned based on ranked weight value. Each mask will correspond to the
    ...
    def __init__(
        ...
        M: int = 4,
        vec_size : int = 64,
        final_vec_sparsity: float = 0.5
    ):
        self._N = N
        self._M = M
        self._V = vec_size
        self._total_sparsity = 0.0
        self._vec_sparsity = final_vec_sparsity

    def create_sparsity_masks(
        self,
        tensors: List[Tensor],  # param_scores
        target: Union[float, List[float]],  # vec_sparsity
        global_sparsity: bool = False,
    ) -> List[Tensor]:
        """
        ...
        masks = []
        self._gyro_sparsity = target[0]
        print("[GEN MASK] Current Sparsity: ", self._total_sparsity)

        if self._total_sparsity < self._vec_sparsity:
            vec_only = True
        else:
            vec_only = False

        for tensor in tensors:
            tensor = torch.where(tensor == -float('inf'),
                                 tensor.new_tensor(0.0), tensor)

            if tensor.numel() % self._V != 0:
                raise ValueError(
                    f"Tensor of size {tensor.shape} can't be evenly divided into "
                    f"{self._M} groups")
            ...
            score_weight = tensor.to("cpu")
            score_weight = score_weight.view(original_shape[0], -1)
            
            if self.perm is not None:
                score_weight = score_weight[self.perm]

            if vec_only:
                perm, thisMask = permutations.apply_out_perm(score=score_weight,
                                                             vec_size=self._V,
                                                             vec_sparsity=self._total_sparsity)
            else:
                thisMask = permutations.apply_in_perm(score=score_weight,
                                                      vec_size=self._V,
                                                      vec_sparsity=self._vec_sparsity,
                                                      n=self._N,
                                                      m=self._M)
            
            if self.perm is not None:
                inverted_perm = torch.argsort(self.perm)
                score_weight = score_weight[inverted_perm]
            
            self.perm = perm
            thisMask = thisMask.reshape(original_shape)
...
def get_mask_creator_default(mask_type: Union[str, List[int]],
    ...
    """
    if mask_type == "hinm":
        return HiNMPruningMaskCreator(vec_size=vec_size,
                                      final_vec_sparsity=final_vec_sparsity)
